...
This page documents known edge cases, common error messages, and their fixes. Each entry follows a **symptom / cause /
fix** format. For quick answers, see the [FAQ](./faq.md).
...
**Symptom:** You see startup-category log messages ("Application registered N route(s)", "Application startup
completed") even though `debug: { startup: false }`.
...
**Cause:** The DI container resolves all services in a module eagerly during `registerModule`. If any constructor
throws, the module registration fails.
...
**Fix:** This is expected behavior. When filtering `getRoutes()`, account for multiple entries per handler. Don't rely
on a 1:1 mapping between handlers and route entries when using `VERSION_NEUTRAL`.
...
**Fix:** Fix the underlying schema/route issues, or relax to `mode: 'best-effort'` / disable the `failOn*` flags during
development.
