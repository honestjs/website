...
Project Status HonestJS is in early development (pre-v1.0.0). The API may change
between minor versions and some features are still in progress. We recommend
caution before using it in production. See
...
Coming from NestJS? The **barebone** template feels most familiar: modules,
controllers, and services are organized the same way. The main differences are
that HonestJS runs on Hono (not Express) and uses `Application.create` instead
of `NestFactory.create`.
