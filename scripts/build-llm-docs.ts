...
async function generateLLMDocs() {
	...
	fs.writeFileSync(
		...
		[
			...
			'## Docs',
			'',
			'- [Full Docs](https://honestjs.dev/llms-full.txt) Full documentation of HonestJS. (without examples)',
			'- [Tiny Docs](https://honestjs.dev/llms-small.txt): Tiny documentation of HonestJS. (includes only desciption of core)',
			'',
			'## Examples',
			'',
			'- [Examples](https://github.com/honestjs/website/tree/main/examples): List of example files.',
			'',
			'## Optional',
			'',
			...optionals,
