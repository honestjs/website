...
The `@Controller()` decorator can take an optional route prefix and configuration options. This prefix will be applied
to all routes defined within that controller.
...
These decorators are used on methods within a controller. They can take an optional path segment and options that will
be appended to the controller's prefix.
...
HonestJS supports flexible API versioning at multiple levels. You can set a global version, a version per controller, or
even a version per route.
